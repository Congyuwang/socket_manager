 ...
 * <li> `ConnCallback` ---strong ref--> (active) `Connection`s (drop on
 * `connection_close`) </li> <li> `Connection` ---strong ref--> `Notifier` </li>
 ...
 * will have a memory leak. The user could `reset()` the
 * `shared_ptr\<MsgSender\>` on connection close event, and thus break the
 * cycle. <br /><br /> In short, the user must guarantee that the `MsgSender`
 * object is released for connection resources to be properly released.
 *
...
public:
   ...
   */
  explicit SocketManager(const std::shared_ptr<ConnCallback> &conn_cb,
                         size_t n_threads = 1);
...
private:
  std::unique_ptr<SOCKET_MANAGER_C_API_SocketManager,
