...
impl CSocketManager {
    ...
    >(
        ...
        let main_handle = runtime.handle().clone();
        let join_handle = Some(std::thread::spawn(move || {
            let handle = main_handle.clone();
            main_handle.block_on(main(cmd_recv, stop, &handle, on_conn, connection_state));
        }));
        Ok(CSocketManager {
            cmd_send,
            cmd_stopper,
    ...
    pub fn join(&mut self) -> std::io::Result<()> {
            ...
            .compare_exchange(false, true, Ordering::Acquire, Ordering::Relaxed)
        {
            Ok(_) => {
                ...
                //this will block until the runtime is stopped
