...
namespace socket_manager {
  ...
  SocketManager::SocketManager(const std::shared_ptr<ConnCallback> &conn_cb, size_t n_threads)
          : conn_cb(conn_cb) {
    char *err = nullptr;
    auto inner_ptr = socket_manager_init(OnConnObj{
            conn_cb.get()
    }, n_threads, &err);
    inner = std::unique_ptr<CSocketManager, std::function<void(CSocketManager *)>>(
            inner_ptr,
            [](CSocketManager *ptr) { socket_manager_free(ptr); }
    );
    if (err) {
