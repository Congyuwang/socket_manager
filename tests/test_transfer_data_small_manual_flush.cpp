...
class StoreAllDataSmallManual : public MsgReceiver {
public:
  explicit StoreAllDataSmallManual(size_t &buffer, int &count) : buffer(buffer), count(count) {}
...
class StoreAllDataSmallManualNotifyOnCloseCallback : public ConnCallback {
public:

  void on_connect(const std::string &local_addr, const std::string &peer_addr,
                  std::shared_ptr<Connection> conn, std::shared_ptr<MsgSender> send) override {
    auto rcv = std::make_unique<StoreAllDataSmallManual>(add_data, count);
...
int test_transfer_data_small_manual_flush(int argc, char **argv) {
  const std::string addr = "127.0.0.1:40013";

  auto send_cb = std::make_shared<SendSmallManualDataConnCallback>();
  auto store_cb = std::make_shared<StoreAllDataSmallManualNotifyOnCloseCallback>();
