...
class StoreAllDataLargeManual : public MsgReceiver {
public:
  explicit StoreAllDataLargeManual(size_t &buffer, int &count) : buffer(buffer), count(count) {}
...
public:
  ...
  void on_connect(const std::string &local_addr, const std::string &peer_addr,
    ...
    auto rcv = std::make_unique<StoreAllDataLargeManual>(add_data, count);
