...
class SendSmallNonBlockDataConnCallback : public DoNothingConnCallback {
public:
...
class StoreAllDataSmallNonBlock : public MsgReceiver {
public:
  explicit StoreAllDataSmallNonBlock(size_t &buffer, int &count) : buffer(buffer), count(count) {}
...
class StoreAllDataSmallNonBlockNotifyOnCloseCallback : public ConnCallback {
public:

  void on_connect(const std::string &local_addr, const std::string &peer_addr,
                  std::shared_ptr<Connection> conn, std::shared_ptr<MsgSender> send) override {
    auto rcv = std::make_unique<StoreAllDataSmallNonBlock>(add_data, count);
...
int test_transfer_data_small_nonblock(int argc, char **argv) {
  const std::string addr = "127.0.0.1:40013";

  auto send_cb = std::make_shared<SendSmallNonBlockDataConnCallback>();
  auto store_cb = std::make_shared<StoreAllDataSmallNonBlockNotifyOnCloseCallback>();
