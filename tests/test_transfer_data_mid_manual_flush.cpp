...
class SendMidManualDataConnCallback : public DoNothingConnCallback {
public:
...
class StoreAllDataMidManual : public MsgReceiver {
public:
  explicit StoreAllDataMidManual(size_t &buffer, int &count) : buffer(buffer), count(count) {}
...
class StoreAllDataMidManualNotifyOnCloseCallback : public ConnCallback {
public:
...
int test_transfer_data_mid_manual_flush(int argc, char **argv) {
  const std::string addr = "127.0.0.1:40013";

  auto send_cb = std::make_shared<SendMidManualDataConnCallback>();
  auto store_cb = std::make_shared<StoreAllDataMidManualNotifyOnCloseCallback>();
