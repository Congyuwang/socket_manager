...
class StoreAllDataSmall : public MsgReceiver {
public:
  explicit StoreAllDataSmall(size_t &buffer, int &count) : buffer(buffer), count(count) {}
...
class StoreAllDataSmallNotifyOnCloseCallback : public ConnCallback {
public:
...
int test_transfer_data_small(int argc, char **argv) {
  const std::string addr = "127.0.0.1:40013";

  auto send_cb = std::make_shared<SendSmallDataConnCallback>();
  auto store_cb = std::make_shared<StoreAllDataSmallNotifyOnCloseCallback>();
