...
class SendLargeNonBlockDataConnCallback : public DoNothingConnCallback {
public:
...
class StoreAllDataLargeNonBlock : public MsgReceiver {
public:
  explicit StoreAllDataLargeNonBlock(size_t &buffer, int &count) : buffer(buffer), count(count) {}
...
public:
  ...
  void on_connect(const std::string &local_addr, const std::string &peer_addr,
    ...
    auto rcv = std::make_unique<StoreAllDataLargeNonBlock>(add_data, count);
...
int test_transfer_data_large_nonblock(int argc, char **argv) {
  ...
  auto send_cb = std::make_shared<SendLargeNonBlockDataConnCallback>();
