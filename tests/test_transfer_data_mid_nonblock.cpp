...
class SendMidNonBlockDataConnCallback : public DoNothingConnCallback {
public:
...
class StoreAllDataMidNonBlock : public MsgReceiver {
public:
  explicit StoreAllDataMidNonBlock(size_t &buffer, int &count) : buffer(buffer), count(count) {}
...
class StoreAllDataMidNonBlockNotifyOnCloseCallback : public ConnCallback {
public:

  void on_connect(const std::string &local_addr, const std::string &peer_addr,
                  std::shared_ptr<Connection> conn, std::shared_ptr<MsgSender> send) override {
    auto rcv = std::make_unique<StoreAllDataMidNonBlock>(add_data, count);
...
int test_transfer_data_mid_nonblock(int argc, char **argv) {
  ...
  auto send_cb = std::make_shared<SendMidNonBlockDataConnCallback>();
  auto store_cb = std::make_shared<StoreAllDataMidNonBlockNotifyOnCloseCallback>();
